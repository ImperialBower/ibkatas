//! Provided working context — the solved **stage 2** sigma proofs, plus the
//! `fiat_shamir_bits` helper the shuffle uses for its challenge bits.
...
/// Derive `rounds` Fiat–Shamir challenge **bits** bound to a sequence of points.
///
/// The points (input deck, output deck, and every published intermediate) are
/// hashed into a seed, which is then expanded in counter mode so the prover
/// cannot pick intermediates after seeing the challenge.
#[must_use]
pub fn fiat_shamir_bits(label: &[u8], points: &[Projective], rounds: usize) -> Vec<bool> {
    ...
    while bits.len() < rounds {
        let mut block = Sha512::new();
        block.update(b"pkmental/shuffle-bits");
        block.update(seed);
